...
class PuzzleSolver:
    ...
    def solve_puzzle_dfs(self):
        ...
        open_list = [self.start] #Start Node
        closed_list = [self.start]
        while open_list:
            cur_node=open_list.pop()
            if cur_node.grid==self.goal.grid:
                self.disp_solution(cur_node)
                return

            closed_list.append(cur_node)

            children = self.generate_children(cur_node)[::-1]

            for child in children:
                if not self.is_in(child,closed_list):
